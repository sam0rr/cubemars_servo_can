...
def clean_can_manager_registry() -> Generator[None]:
    """Guarantee CAN manager isolation and cleanup for every test."""
    for manager in tuple(CanManagerRegistry._managers.values()):
        manager.close()
    CanManagerRegistry._managers.clear()
    yield
    for manager in tuple(CanManagerRegistry._managers.values()):
        manager.close()
    CanManagerRegistry._managers.clear()
